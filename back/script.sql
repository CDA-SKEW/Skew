...
CREATE TABLE IF NOT EXISTS `Skew`.`user` (
  ...
  `date_update` TIMESTAMP(20) NOT NULL ON UPDATE CURRENT_TIMESTAMP,
  `date_create` TIMESTAMP(20) NOT NULL DEFAULT DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `Skew`.`offre` (
  ...
  `createDate` TIMESTAMP(20) NULL DEFAULT DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS `Skew`.`experience` (
  ...
  `dateStart` TIMESTAMP(20) NULL,
  `dateEnd` TIMESTAMP(20) NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `fk_experiences_user1_idx` (`user_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `Skew`.`document` (
  ...
  CONSTRAINT `fk_aptitudes_user11`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_document_postuled1`
    FOREIGN KEY (`id`)
    REFERENCES `Skew`.`postuled` (`document_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
